...
func newGatewayFixture(t *testing.T, enabled bool, writer HHWriteClient) (*HHWriteGateway, *gatewayReadFake, *gatewayWriteFake, EmployerConversation, AIDraft) {
	...
	gateway := NewHHWriteGateway(HHWriteGatewayOptions{Enabled: enabled, RequireFreshRead: true, Client: writer, ReadClient: reader, Conversations: conversations, Drafts: drafts, Resolver: resolver, Actions: NewApprovedHHActionStore(filepath.Join(dir, HHWriteActionsFilename)), Audit: NewHHWriteAuditStore(filepath.Join(dir, HHWriteEventsFilename))})
...
func TestHHWriteGatewayDryRunFreshValidationDoesNotConsumeNonceOrMetrics(t *testing.T) {
	...
	if reader.reads != 1 || writer.calls != 0 {
		t.Fatalf("dry-run did not use read-only fresh validation exactly once: reads=%d writes=%d", reader.reads, writer.calls)
	}
}
