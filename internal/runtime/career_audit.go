...
func readCareerAuditSnapshot(wd, syncPath string) CareerSnapshot {
	...
	if err := clarifications.Load(); err != nil && !errors.Is(err, os.ErrNotExist) {
		d.StoreErrors = append(d.StoreErrors, "candidate_clarifications.json: unreadable or invalid JSON")
	} else if values, listErr := clarifications.List(); listErr == nil {
		d.Clarifications = values
	}
	if syncPath == "" {
