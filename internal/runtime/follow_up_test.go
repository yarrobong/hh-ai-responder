...
func (f *followUpRaceAI) ChatStructuredWithSchema(_ string, _ string, _ int, _ float64, _ *ChatJSONSchema, _ func(string) error) (string, error) {
	f.calls++
	if f.calls == 1 {
		at := time.Date(2026, 9, 10, 11, 0, 0, 0, time.UTC)
		if _, err := f.store.AppendMessage(f.conversationID, ConversationMessage{ID: "candidate-race", ExternalID: "candidate-race", Timestamp: at, Sender: ConversationSenderCandidate, Source: ConversationSourceHH, Direction: ConversationOutgoing, Text: "Ответил работодателю вручную."}); err != nil {
			return "", err
		}
	}
...
func TestFollowUpFreshRecheckDiscardsDraftAfterCandidateReply(t *testing.T) {
	s := dashboardTestServer(t)
	in, now := followUpFixture()
	s.Applications.applications = []JobApplication{in.Application}
	s.Conversations.conversations = []EmployerConversation{in.Conversation}
	ai := &followUpRaceAI{
		store: s.Conversations, conversationID: in.Conversation.ID,
		response: validOrchestratorDecision("draft_reply", "Короткий follow-up.", nil, nil),
	}
	s.Orchestrator.ai = ai
	decision, err := s.Orchestrator.PrepareFollowUp(in.Application.ID, DefaultFollowUpPolicy(), now)
	requireKnowledgeOK(t, err)
	if decision.Action != AIActionManualReview || ai.calls != 1 || len(s.Drafts.drafts) != 0 {
		t.Fatalf("stale follow-up was not discarded: decision=%+v calls=%d drafts=%d", decision, ai.calls, len(s.Drafts.drafts))
	}
}
