...
func TestStage22BoundedConversationReadLimitsDetailExpansion(t *testing.T) {
	c := stage22HTTPClient(t, 40, 0)
	tr := stage22Trace(c)
	page, err := c.ReadConversationsBounded(context.Background(), "", 3)
	if err != nil {
		t.Fatal(err)
	}
	if len(page.Items) != 3 || page.DetailedChatsFetched != 3 || tr.lists.Load() != 1 || tr.calls.Load() != 4 {
		t.Fatalf("bounded page items=%d details=%d listCalls=%d requests=%d", len(page.Items), page.DetailedChatsFetched, tr.lists.Load(), tr.calls.Load())
	}
}
func TestStage22Read429BackoffAndNoWriteRetry(t *testing.T) {
	...
	if _, err := requester.Do(req); err == nil {
		t.Fatal("shared HH read requester allowed a state-changing method")
	}
	if calls.Load() != 1 {
		t.Fatal("blocked write reached the network")
	}
}
...
func TestStage22CoalescesFullAndTargetedSync(t *testing.T) {
	for _, target := range []string{"conversations", "conversation:12"} {
		t.Run(target, func(t *testing.T) {
			...
			svc := NewHHReadSyncServiceWithOptions(client, HHReadSyncServiceOptions{Vacancies: v, Applications: a, Conversations: c, StatePath: path})
			var wg sync.WaitGroup
			wg.Add(1)
			go func() {
				defer wg.Done()
...
func TestStage22BatchMergePreservesExternalEvidence(t *testing.T) {
	v, a, c, path := newSyncStores(t)
	svc := NewHHReadSyncServiceWithOptions(&syncFakeReadClient{}, HHReadSyncServiceOptions{Vacancies: v, Applications: a, Conversations: c, StatePath: path})
...
func TestStage22BatchDoesNotAnalyzeUnderFileLock(t *testing.T) {
	...
	svc := NewHHReadSyncServiceWithOptions(&syncFakeReadClient{}, HHReadSyncServiceOptions{Vacancies: v, Applications: a, Conversations: c, Analyzer: analyzer, StatePath: path})
	result := SyncResult{}
