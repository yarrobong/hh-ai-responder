...
func (r *MemoryCandidateSemanticRepository) Search(_ context.Context, query CandidateSemanticSearchQuery) ([]CandidateSemanticResult, error) {
	...
	items := []scored{}
	for _, doc := range r.documents {
		if doc.CandidateID != query.CandidateID || doc.EmbeddingSpaceID != query.EmbeddingSpaceID || doc.EmbeddingProvider != query.EmbeddingProvider || doc.EmbeddingModel != query.EmbeddingModel || doc.EmbeddingDimensions != query.EmbeddingDimensions {
			continue
		}
...
func (r *MemoryCandidateSemanticRepository) EmbeddingSpaceMatches(_ context.Context, candidateID string, contract semantic.EmbeddingContract) error {
	if r == nil {
		return errors.New("memory semantic repository is not configured")
	}
	for _, doc := range r.documents {
		...
		if doc.EmbeddingSpaceID != contract.SpaceID || doc.EmbeddingProvider != contract.Provider || doc.EmbeddingModel != contract.Model || doc.EmbeddingDimensions != contract.Dimensions {
			return &semantic.SemanticIndexIncompatibilityError{CandidateID: candidateID, ActiveSpace: contract.SpaceID, FoundSpace: doc.EmbeddingSpaceID}
		}
	}
	return nil
}

var _ ports.CandidateSemanticSpaceInspector = (*MemoryCandidateSemanticRepository)(nil)
...
func validateMemorySemanticDocument(document CandidateSemanticDocument) error {
	if err := semantic.ValidateVector(document.Embedding, document.EmbeddingDimensions, "memory semantic document"); err != nil {
		return err
	}
	if document.CandidateID == "" || document.EntityID == "" || document.ContentHash == "" || document.EmbeddingProvider == "" || document.EmbeddingModel == "" || document.EmbeddingSpaceID == "" {
