...
func TestAutoRespondChatsUsesProposalBoundaryAndKeepsWritesDisabled(t *testing.T) {
	previousLogger := logger
	logger = NewLogger(io.Discard, LevelDebug)
	t.Cleanup(func() { logger = previousLogger })
	run := func(t *testing.T, completion string) (aiCalls, writeCalls int32, events string) {
		t.Helper()
		var ai, writes atomic.Int32
		...
		server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
			...
			case "/chatik/api/chats":
				if request.Method != http.MethodGet {
					t.Fatalf("chat-list method=%s, want GET", request.Method)
				}
				query := request.URL.Query()
				for key, want := range map[string]string{"do_not_track_session_events": "true", "filterHasTextMessage": "false", "filterUnread": "false"} {
					if got := query.Get(key); got != want {
						t.Fatalf("chat-list query %s=%q, want %q", key, got, want)
					}
				}
				if len(query) != 3 {
					t.Fatalf("chat-list query has unexpected parameters: %v", query)
				}
				if got := request.URL.Query().Get("from"); got != "" {
					t.Fatalf("first chat-list request sent invalid from cursor %q", got)
				}
				if request.Header.Get("X-Requested-With") != "XMLHttpRequest" || request.Header.Get("Accept") != "application/json" || request.Header.Get("Authorization") != "" {
					t.Fatalf("chat-list headers do not preserve GET-only contract")
				}
				_, _ = fmt.Fprintf(w, `{"chats":{"pages":1,"items":[{"id":7,"resources":{"VACANCY":["v1"],"RESUME":["1"]},"lastMessage":{"id":2,"creationTime":%q,"text":"Здравствуйте, есть ли опыт?","type":"SIMPLE","participantId":"99","participantDisplay":{"name":"HR"}}}]},"resources":{"resumes":{"1":{}},"vacancies":{"v1":{"vacancyId":42,"name":"Backend","company":{"name":"Example"},"links":{"desktop":"%s"},"compensation":{"from":100,"currencyCode":"RUR"}}}}}`, created, server.URL)
			case "/chatik/api/chat_data":
				_, _ = fmt.Fprintf(w, `{"chat":{"id":7,"currentParticipantId":"17","messages":{"items":[{"id":2,"creationTime":%q,"text":"Здравствуйте, есть ли опыт?","type":"SIMPLE","participantId":"99","participantDisplay":{"name":"HR"}}]},"writePossibility":{"allowed":true}},"resources":{"participants":{"99":{"type":"EMPLOYER_USER"}}}}`, created)
			case "/v1/chat/completions":
				ai.Add(1)
				_, _ = fmt.Fprintf(w, `{"choices":[{"message":{"role":"assistant","content":%q},"finish_reason":"stop"}]}`, completion)
			case "/chatik/api/send", "/chatik/api/leave":
				writes.Add(1)
				t.Fatalf("write endpoint called: %s", request.URL.Path)
			default:
		...
		responder := &HHAIResponder{
			ctx: ctx, baseURL: mustURL(t, server.URL), chatURL: server.URL, client: server.Client(),
			ai:         NewAIClient(ctx, server.URL, "model", "", time.Second, time.Second, 1),
			resumes:    []ResumeItem{{Id: 1, Hash: "resume-hash", Title: "Python developer", Salary: "50000 руб"}},
			resumeHash: "resume-hash", userId: 17, autoChat: true, chatMode: "auto", dryRun: true,
			eventWriter: &eventLog,
		}
		if err := responder.AutoRespondChats(); err != nil {
			t.Fatal(err)
		}
		return ai.Load(), writes.Load(), eventLog.String()
	}

	aiCalls, writeCalls, events := run(t, "Готов продолжить.")
	if aiCalls != 1 || writeCalls != 0 || !strings.Contains(events, `"chat_reply_preview"`) || !strings.Contains(events, `"dry-run"`) {
		t.Fatalf("safe proposal did not follow root dry-run gate: ai=%d writes=%d events=%s", aiCalls, writeCalls, events)
	}
	aiCalls, writeCalls, events = run(t, "Тестовое задание выполнено.")
	if aiCalls != 1 || writeCalls != 0 || !strings.Contains(events, `"chat_reply_preview"`) {
		t.Fatalf("manual-review proposal reached an unexpected path: ai=%d writes=%d events=%s", aiCalls, writeCalls, events)
	}
}
...
func TestLegacyMaintenanceGatewayHonorsWriteEnabled(t *testing.T) {
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
	...
	responder := &HHAIResponder{
		...
		jar:                   jar,
		resumeHash:            "resume-hash",
		resumeProfileFrontURL: server.URL,
		autoTouch:             true,
		autoJobStatus:         true,
	}

	if updated, err := responder.TouchResume(); err == nil || updated {
		t.Fatalf("write-disabled resume touch was not blocked: updated=%v err=%v", updated, err)
	}
	if updated, err := responder.SetActiveJobSearchStatus(); err == nil || updated {
		t.Fatalf("write-disabled job-status update was not blocked: updated=%v err=%v", updated, err)
	}
	if got := calls.Load(); got != 0 {
		t.Fatalf("write-disabled maintenance issued %d HH requests", got)
	}

	responder.hhWriteEnabled = true
	if updated, err := responder.TouchResume(); err != nil || !updated {
		t.Fatalf("enabled resume touch failed: updated=%v err=%v", updated, err)
	}
	if updated, err := responder.SetActiveJobSearchStatus(); err != nil || !updated {
		t.Fatalf("enabled job-status update failed: updated=%v err=%v", updated, err)
	}
	if got := calls.Load(); got != 2 {
		t.Fatalf("enabled maintenance made %d HH requests, want 2", got)
	}
}
