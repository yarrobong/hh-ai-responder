...
// application logic and JSON/PostgreSQL repository bundles.
...
func (r *CareerDataReconciler) postgresCareerStore() *PostgresCareerStore {
	...
	vacancyPool := vacancies.Pool()
	if !vacanciesOK || !applicationsOK || !conversationsOK || vacancies == nil || applications == nil || conversations == nil || vacancyPool == nil || vacancyPool != applications.Pool() || vacancyPool != conversations.Pool() {
		return nil
	}
	return NewPostgresCareerStore(vacancyPool)
}
