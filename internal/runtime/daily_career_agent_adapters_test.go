...
func TestDailyVacancyStageMapsExistingCareerAgentReport(t *testing.T) {
	...
	if result.Summary.Vacancy.Found != 4 || !result.Summary.Vacancy.RawHitsKnown || !result.Summary.Vacancy.DiagnosticsKnown || result.Summary.Vacancy.New != 3 || result.Summary.Vacancy.Matched != 1 || result.Summary.Vacancy.ReviewRequired != 1 || !result.Summary.AI.Known || result.Summary.AI.Requested != 2 || result.Summary.Vacancy.RouteAmbiguous != 2 || result.Summary.Vacancy.RouteLowEvidence != 3 || result.Summary.Vacancy.RoleOutOfScope != 5 || result.Summary.Vacancy.NoSuitableResume != 4 || result.Summary.Vacancy.HardUnknown != 1 {
