...
func writeCommandHelp(command appcli.CommandKind, out io.Writer) error {
	usage := map[appcli.CommandKind]string{
		...
		appcli.CommandHHAPI:       "usage: hh-api auth | doctor | logout | preflight <vacancy-id> [--resume-id ID] [--all-resumes] | approval export --pilot <path> --out <path> | apply <vacancy-id> --resume-id <provider-id> --approval-file <path>",
