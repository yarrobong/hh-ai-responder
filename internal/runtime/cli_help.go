...
func writeCommandHelp(command appcli.CommandKind, out io.Writer) error {
	usage := map[appcli.CommandKind]string{
		...
		appcli.CommandCareerAgent: "usage: career-agent daily [--json] | career-agent run | career-agent --shadow | career-agent --canary | career-agent browser-doctor [--headless|--headed] | career-agent resumes | career-agent resume enable|disable --id <resume-id> | career-agent feedback --vacancy <id> --type <type> | career-agent pilot --search [--max-scan 1..1000] [--max-candidates 1..20] | career-agent pilot --vacancy <id> [--resume-id <identity>] [--omit-optional-cover-letter] | career-agent pilot send --nonce <nonce> | career-agent web-trace --known <id,id> --unknown <id,id,...>",
		appcli.CommandHHDoctor:    "usage: hh-doctor (GET/read-only HH access diagnostics)",
		appcli.CommandHHAPI:       "usage: hh-api auth | doctor | logout | preflight <vacancy-id> [--resume-id ID] [--all-resumes] | approval export --pilot <path> --out <path> | approval review --pilot <path> --out <path> [--letter-file <path>] | apply <vacancy-id> --resume-id <provider-id> --approval-file <path> | apply-batch --approval-file <path> [--approval-file <path> ...]",
