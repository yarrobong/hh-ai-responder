...
func TestR145dReliabilityNotificationLifecycleDoesNotTouchAttemptAuthority(t *testing.T) {
	store := NewNotificationStore(filepath.Join(t.TempDir(), NotificationEventsFilename))
	if err := store.Load(); err != nil {
		t.Fatal(err)
	}
	projector := reliabilitynotifications.NewProjector(store)
	if err := projector.ProjectApplicationOutcome(context.Background(), reliabilitynotifications.ApplicationOutcome{AttemptID: "A1", VacancyID: 42, State: "DELIVERY_UNCERTAIN"}); err != nil {
		t.Fatal(err)
	}
	before := store.List()[0]
	if err := store.Dismiss(before.ID); err != nil {
		t.Fatal(err)
	}
	after := store.List()[0]
	if after.Lifecycle != NotificationDismissed || after.RelatedAttemptID != "A1" || after.RelatedVacancyID != 42 {
		t.Fatalf("unexpected dismissed notification: %+v", after)
	}
	if after.RelatedAttemptID != before.RelatedAttemptID || after.RelatedVacancyID != before.RelatedVacancyID {
		t.Fatal("notification dismissal changed incident identity")
	}
}

func TestR145dConfirmationWithoutPriorNotificationRemainsVisible(t *testing.T) {
	store := NewNotificationStore(filepath.Join(t.TempDir(), NotificationEventsFilename))
	if err := store.Load(); err != nil {
		t.Fatal(err)
	}
	projector := reliabilitynotifications.NewProjector(store)
	if err := projector.ProjectApplicationReconciliation(context.Background(), reliabilitynotifications.ApplicationReconciliation{AttemptID: "A2", VacancyID: 43, Status: "CONFIRMED"}); err != nil {
		t.Fatal(err)
	}
	items := store.List()
	if len(items) != 1 || items[0].Lifecycle != NotificationNew || items[0].Message != "Отклик подтверждён на HH." {
		t.Fatalf("confirmation was hidden or misstated: %+v", items)
	}
}
