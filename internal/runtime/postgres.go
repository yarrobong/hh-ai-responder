...
// careerRepositoriesFromStores exposes an already-composed backend to a
// second service in the same process. It prevents dashboard/monitor/CLI
// assembly from opening a second pool or accidentally rebuilding JSON
// repositories after the backend has been selected.
func careerRepositoriesFromStores(vacancies *VacancyStore, applications *ApplicationStore, conversations *ConversationStore) (CareerRepositories, bool) {
	if vacancies == nil || applications == nil || conversations == nil || vacancies.careerRepository == nil || applications.careerRepository == nil || conversations.careerRepository == nil {
		return CareerRepositories{}, false
	}
	career := CareerRepositories{Vacancies: vacancies.careerRepository, Applications: applications.careerRepository, Conversations: conversations.careerRepository}
	if vacancyRepo, ok := vacancies.careerRepository.(*PostgresVacancyRepository); ok {
		if pool := vacancyRepo.Pool(); pool != nil {
			career.Postgres = NewPostgresCareerStore(pool)
		}
	}
	return career, true
}
...
func normalizeStorageBackend(value string) (string, error) {
	return appconfig.NormalizeStorageBackend(value)
}
