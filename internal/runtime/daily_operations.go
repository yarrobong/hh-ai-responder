...
func (s *DashboardServer) refreshDailyOperations(now time.Time, inbox CandidateInbox, syncResult SyncResult) (DailyRefreshRun, error) {
	...
	return s.persistDailyOperations(now, inbox, syncResult)
}

// persistDailyOperations keeps the daily-state write phase separate from
// notification calculation so inboxrefresh can preserve the existing order
// while owning the one-iteration orchestration boundary.
func (s *DashboardServer) persistDailyOperations(now time.Time, inbox CandidateInbox, syncResult SyncResult) (DailyRefreshRun, error) {
	if now.IsZero() {
		now = time.Now().UTC()
	}
