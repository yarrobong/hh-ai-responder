...
func TestRenderDailyCareerAgentStatusIsStableAndExplicitlyReadOnly(t *testing.T) {
	...
	for _, expected := range []string{"Career Agent daily: PARTIAL_SUCCESS", "Run: daily-career-agent-2026-09-24", "Vacancies: processed=4 raw_hits=3 matched=1 review=2", "AI reviewed: 0", "Prepared: 0", "Rejected: 0", "Route ambiguous: 0", "Low evidence: 0", "Role out of scope: 0", "Hard unknown: 0", "No suitable resume: 0", "Attention breakdown: application_ready=0 vacancy_review=0 clarifications=0 needs_reply=0 interviews=0 tests=0 offers=0 follow_ups=0 other=0", "Communication: conversations=5 new_messages=7 replies_needed=2 failures=1", "Attention: 2", "HH writes: 0", "Replay: true"} {
		if !strings.Contains(output, expected) {
