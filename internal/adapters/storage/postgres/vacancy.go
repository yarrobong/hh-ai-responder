...
// Vacancy and VacancyQuery aliases keep the adapter implementation readable
// while its public methods still expose the shared domain and port types.
type Vacancy = vacancy.Vacancy
type VacancyQuery = ports.VacancyQuery

// postgresDBTX is intentionally private to this physical adapter. Both a
// connection pool and a transaction satisfy it without leaking pgx through
// the domain or persistence ports.
...
// Pool is a composition-only inspection hook used by the current root
// transaction/reconciliation wiring. It is not part of the vacancy port.
func (r *VacancyRepository) Pool() *pgxpool.Pool {
	if r == nil {
		return nil
	}
	return r.pool
...
func NewVacancyRepository(pool *pgxpool.Pool) *VacancyRepository {
	...
	return &VacancyRepository{pool: pool, db: db}
}

func NewVacancyRepositoryForTx(tx pgx.Tx) *VacancyRepository {
	return &VacancyRepository{db: tx}
}

func (r *VacancyRepository) Get(ctx context.Context, id int) (Vacancy, error) {
...
func (r *VacancyRepository) GetByExternalID(ctx context.Context, externalID string) (Vacancy, error) {
	ctx = postgresContext(ctx)
	if err := repositoryContextErr(ctx); err != nil {
		return Vacancy{}, err
	}
...
func (r *VacancyRepository) List(ctx context.Context, query VacancyQuery) ([]Vacancy, error) {
	ctx = postgresContext(ctx)
	if err := repositoryContextErr(ctx); err != nil {
		return nil, err
	}
...
func (r *VacancyRepository) Create(ctx context.Context, value Vacancy) (Vacancy, error) {
	ctx = postgresContext(ctx)
	if err := repositoryContextErr(ctx); err != nil {
		return Vacancy{}, err
	}
...
func (r *VacancyRepository) Import(ctx context.Context, value Vacancy) error {
	ctx = postgresContext(ctx)
	if err := repositoryContextErr(ctx); err != nil {
		return err
	}
...
func (r *VacancyRepository) Update(ctx context.Context, value Vacancy) error {
	ctx = postgresContext(ctx)
	if err := repositoryContextErr(ctx); err != nil {
		return err
	}
...
func (r *VacancyRepository) requirePool() error {
	if r == nil || r.db == nil {
...
func (r *VacancyRepository) beginMutation(ctx context.Context) (pgx.Tx, bool, error) {
	if r == nil || r.db == nil {
...
func vacancyArgs(value Vacancy) ([]interface{}, error) {
...
func nullableJSON(value interface{}) ([]byte, error) {
	if value == nil {
		return nil, nil
	}
...
func nullableUnixNano(value time.Time) interface{} {
	if value.IsZero() {
		return nil
	}
...
func decodeNullableJSON(raw []byte, target interface{}) error {
...
func cloneVacancy(value Vacancy) (Vacancy, error) {
	raw, err := json.Marshal(value)
	if err != nil {
		return Vacancy{}, err
	}
	...
	copy.TotalResponsesCountKnown = value.TotalResponsesCountKnown
	return copy, nil
}
