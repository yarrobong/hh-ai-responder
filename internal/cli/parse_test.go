...
func TestParseSupportedInvocations(t *testing.T) {
	tests := []struct {
		...
		{name: "hh api apply batch", args: []string{"hh-api", "apply-batch", "--approval-file", "one.json", "--approval-file=two.json"}, command: CommandHHAPI, subcommand: "apply-batch", commandArgs: []string{"apply-batch", "--approval-file", "one.json", "--approval-file=two.json"}},
...
func TestParseRejectsHHAPIExtraPositionalArguments(t *testing.T) {
	for _, args := range [][]string{
		...
		{"hh-api", "apply"},
		{"hh-api", "apply", "42", "--resume-id", "resume-1"},
		{"hh-api", "apply", "42", "--approval-file", "approval.json"},
		{"hh-api", "apply", "42", "--resume-id", "resume-1", "--approval-file", "approval.json", "extra"},
		{"hh-api", "apply", "42", "--resume-id", "resume-1", "--approval-file", "approval.json", "--approval-file", "other.json"},
		{"hh-api", "apply", "42", "--resume-id", "resume-1", "--approval-file", "approval.json", "--default-approval"},
		{"hh-api", "apply-batch"},
		{"hh-api", "apply-batch", "--approval-file", "one.json", "--approval-file", "two.json", "--approval-file", "three.json", "--approval-file", "four.json"},
		{"hh-api", "approval", "review"},
		{"hh-api", "approval", "review", "--pilot", "pilot.json"},
		{"hh-api", "approval", "review", "--out", "approval.json"},
		{"hh-api", "approval", "review", "--pilot", "pilot.json", "--out", "approval.json", "--letter-file", "letter.txt", "--letter-file", "other.txt"},
		{"hh-api", "approval", "review", "--pilot", "pilot.json", "--out", "approval.json", "--unknown", "value"},
