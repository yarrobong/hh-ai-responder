...
func (g *HHWriteGateway) writesToday(now time.Time) int {
	if g == nil || g.Audit == nil {
		return 0
	}
	year, month, day := now.UTC().Date()
	count := 0
	for _, event := range g.Audit.List() {
		ey, em, ed := event.CreatedAt.UTC().Date()
		if event.Type == "send_started" && ey == year && em == month && ed == day {
			count++
		}
	}
	return count
...
func (g *HHWriteGateway) writeLimitError(now time.Time) error {
	if g == nil {
		return errors.New("HH Write Gateway is unavailable")
	}
	if g.MaxWritesPerRun > 0 && g.writesThisRun >= g.MaxWritesPerRun {
		return errors.New("HH write run limit reached")
	}
	if g.MaxWritesPerDay > 0 && g.writesToday(now) >= g.MaxWritesPerDay {
