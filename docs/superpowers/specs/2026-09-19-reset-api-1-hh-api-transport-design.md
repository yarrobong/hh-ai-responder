...
- selected resume detail read: `GET https://api.hh.ru/resumes/{resume_id}`;
...
| Own resumes and selected resume facts | `GET /resumes/mine`, `GET /resumes/{resume_id}` | `API_SUPPORTED` | API resume summaries and selected resume details map to the existing read-only resume/profile inputs; no resume mutation. |
...
| Own resumes and selected resume facts | `API_SUPPORTED` | Browser resume read | Implement read-only API normalization |
