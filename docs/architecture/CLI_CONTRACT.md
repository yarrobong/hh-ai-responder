...
| `hh-api` | `auth`, `doctor`, `logout`, `preflight VACANCY_ID [--resume-id ID] [--all-resumes]`, `approval export --pilot PATH --out PATH`, `approval review --pilot PATH --out PATH [--letter-file PATH]`, `apply VACANCY_ID --resume-id ID --approval-file PATH` | environment/dotenv application config | approval export and review are local/read-only and atomically write focused private artifacts; review is only an explicit operator approval for AI-uncertain manual review; preflight is authenticated GET-only diagnostics; apply is the explicit controlled API mutation path and never follows POST redirects or retries | `runHHAPICommand` |
