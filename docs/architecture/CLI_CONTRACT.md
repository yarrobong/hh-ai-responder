...
| `hh-api` | `auth`, `doctor`, `logout`, `preflight VACANCY_ID [--resume-id ID] [--all-resumes]`, `approval export --pilot PATH --out PATH`, `apply VACANCY_ID --resume-id ID --approval-file PATH` | environment/dotenv application config | approval export is local/read-only and atomically writes a focused private artifact; preflight is authenticated GET-only diagnostics; apply is the explicit controlled API mutation path and never follows POST redirects or retries | `runHHAPICommand` |
