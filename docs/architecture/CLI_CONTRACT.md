...
| `hh-api` | `auth`, `doctor`, `logout`, `preflight VACANCY_ID [--resume-id ID] [--all-resumes]` | environment/dotenv application config | authenticated HH API GET diagnostics; preflight reports documented application-attempt eligibility, never guarantees POST acceptance; suitable-resume matrix is read-only and never routes or switches resumes | `runHHAPICommand` |
