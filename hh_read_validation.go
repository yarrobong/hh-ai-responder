...
func (c *HHAIResponderReadClient) readChatsContext(ctx context.Context, cursor string) (*ChatsResponse, error) {
	...
	req, err := r.buildRequest(http.MethodGet, endpoint, nil, map[string]string{"Accept": "application/json", "X-Requested-With": "XMLHttpRequest", "X-Xsrftoken": r.XSRFToken()})
	if err != nil {
		return nil, err
	}
	...
	if json.Unmarshal(resp.Body, &data) != nil || data.Chats.Items == nil {
		return nil, errors.New("invalid HH chats response")
	}
	return &data, nil
}
...
func mergeHHConversation(old EmployerConversation, incoming *EmployerConversation) {
	...
	incoming.Summary = old.Summary
	incoming.FollowUpState = old.FollowUpState
	incoming.NextAction = old.NextAction
	merged := append([]ConversationMessage{}, old.Messages...)
	for _, m := range incoming.Messages {
		found := false
		for j, prior := range old.Messages {
			if m.Source == prior.Source && m.ExternalID != "" && m.ExternalID == prior.ExternalID {
				found = true
				if m.HHSystemEvent && prior.Text == "" && m.Timestamp.Equal(prior.Timestamp) {
					merged[j].HHSystemEvent = true
				} else if !sameConversationMessage(m, prior) {
	...
	// Preserve unresolved import conflicts across later syncs.
	for k, v := range old.HHMetadata {
		if (k == "warning_message_content_conflict" || k == "sync_list_fingerprint") && incoming.HHMetadata[k] == "" {
			incoming.HHMetadata[k] = v
		}
	}
}
