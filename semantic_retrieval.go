...
// SemanticRetrievalPurpose identifies the production flow that requested a
// semantic lookup. It is intentionally typed so a caller cannot smuggle an
// unrelated, unbounded context map into the retrieval path.
type SemanticRetrievalPurpose string

const (
	SemanticRetrievalPurposeEmployerReply SemanticRetrievalPurpose = "employer_reply"
	SemanticRetrievalPurposeCoverLetter   SemanticRetrievalPurpose = "cover_letter"
	SemanticRetrievalPurposeVacancy       SemanticRetrievalPurpose = "vacancy"
)
...
type SemanticRetrievalRequest struct {
	...
	Limit       int
	MinScore    *float64
	Purpose     SemanticRetrievalPurpose
}

type CandidateSemanticRetriever interface {
	Retrieve(context.Context, SemanticRetrievalRequest) ([]CandidateSemanticResult, error)
}

func (s *CandidateSemanticSearchService) Retrieve(ctx context.Context, request SemanticRetrievalRequest) ([]CandidateSemanticResult, error) {
	if s == nil {
	...
	if minScore == nil {
		defaultScore := semanticRetrievalDefaultMinScore
		minScore = &defaultScore
	}
	return s.Search(ctx, CandidateSemanticQuery{
		CandidateID: request.CandidateID,
		Query:       request.Query,
		EntityTypes: append([]CandidateSemanticEntityType{}, request.EntityTypes...),
		Limit:       request.Limit,
		MinScore:    minScore,
	})
}
...
const (
	...
	semanticRetrievalDefaultMinScore = 0.55
)
