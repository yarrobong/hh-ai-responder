...
func runCandidateSemanticCommand(args []string, cfg Config, out io.Writer) error {
	...
	if subcommand == "status" {
		...
		return printCandidateSemanticStatus(ctx, p, candidateID, out)
	}
	...
	provider, providerErr := configuredEmbeddingProvider(cfg)
	if providerErr != nil && subcommand == "search" {
		return providerErr
	}
...
func printCandidateSemanticStatus(ctx context.Context, pool *pgxpool.Pool, candidateID string, out io.Writer) error {
	...
	model := ""
	if len(documents) > 0 {
		model = documents[0].EmbeddingModel
	}
	fmt.Fprintf(out, "Embedding model: %s\nIndex status: %s\n", firstNonEmpty(model, "not indexed"), map[bool]string{true: "stale", false: "ready"}[len(documents) == 0 || hasSemanticStatusStale(drafts, byKey, candidateID)])
	return nil
}

func hasSemanticStatusStale(drafts []CandidateSemanticDocumentDraft, documents map[string]CandidateSemanticDocument, candidateID string) bool {
	...
	for _, draft := range drafts {
		...
		if draft.Eligible && (!ok || old.ContentHash != draft.ContentHash) {
			return true
		}
	}
...
func runCandidateStatusCommand(args []string, cfg Config, out io.Writer) error {
	...
	return json.NewEncoder(out).Encode(map[string]any{"candidate_storage": "postgres", "candidate_id": candidate.ID, "schema": "ready", "candidate_version": candidate.Version, "legacy_json": "compatibility_only"})
}
