...
func TestEmployerReplyOrchestratorUsesUnifiedStructuredProfileKnowledge(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			decision, err := NewAIReplyOrchestrator(ai, builder, drafts).PrepareEmployerReply(conversation.ID)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTerminalConversationPolicyIsSharedAcrossStateEligibilityPilotAndNotifications(t *testing.T) {
	...
	decision, err := NewAIReplyOrchestrator(ai, NewConversationContextBuilder(store, resolver)).PrepareEmployerReply(c.ID)
	...
	inbox, err := NewHHReadSyncService(nil, store).GetCandidateInbox()
