...
func TestReconcileUsesStrongIDsAndPreservesProvenance(t *testing.T) {
	...
	r := NewCareerDataReconciler(vacancies, applications, conversations)
...
func TestReconcileDoesNotUseSimilarNames(t *testing.T) {
	...
	result, err := NewCareerDataReconciler(vacancies, applications, conversations).Reconcile(true)
...
func TestReconcileCreatesPartialNegotiationAndSkipsPartialMatch(t *testing.T) {
	...
	result, err := NewCareerDataReconciler(vacancies, applications, conversations).Reconcile(false)
...
func TestCareerMonitorNotificationIsIdempotentAndReadOnly(t *testing.T) {
	...
	syncService := NewHHReadSyncService(monitorFixtureClient{conversation: HHConversationRecord{ExternalID: "chat-1", VacancyID: 42, Company: "Company", VacancyTitle: "Backend", CreatedAt: now, UpdatedAt: now, Messages: []HHMessageRecord{{ExternalID: "message-1", Sender: "employer", Text: "Hello", Timestamp: now}}}}, vacancies, applications, conversations, filepath.Join(t.TempDir(), HHSyncStateFilename))
	engine := NewCandidateNotificationEngine(notifications, applications, conversations, nil, DefaultFollowUpPolicy(), time.Minute)
	monitor := NewCareerMonitor(syncService, NewCareerDataReconciler(vacancies, applications, conversations), engine, applications, conversations, filepath.Join(t.TempDir(), CareerMonitorStateFilename), time.Hour)
