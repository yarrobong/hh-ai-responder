...
func (s *DashboardServer) servePage(w http.ResponseWriter, r *http.Request) {
	...
	default:
		valid := path == "/health" || path == "/" || path == "/today" || path == "/inbox" || path == "/applications" || path == "/vacancies" || path == "/knowledge" || path == "/knowledge/questions" || path == "/analytics" || path == "/sync"
...
func (s *DashboardServer) writeAPI(w http.ResponseWriter, r *http.Request, p []string) {
	...
	case "knowledge":
		...
		case "clarifications":
			...
			result, err = s.Orchestrator.ResolveCandidateClarification(p[2], body.Answer)
		case "unknowns":
