...
func stage22LocalDashboard(tb testing.TB) *DashboardServer {
	...
	tb.Logf("startup=%s conversations=%d messages=%d applications=%d vacancies=%d actions=%d audit=%d", time.Since(start), len(s.Conversations.conversations), stage22MessageCount(s.Conversations), len(s.Applications.applications), len(s.Vacancies.vacancies), len(s.WriteGateway.Actions.List()), len(s.WriteGateway.Audit.List()))
...
func BenchmarkStage22BatchMerge(b *testing.B) {
	...
	svc := NewHHReadSyncService(&syncFakeReadClient{}, c, filepath.Join(dir, HHSyncStateFilename))
...
func BenchmarkStage22Reconciliation(b *testing.B) {
	s := stage22LocalDashboard(b)
	r := NewCareerDataReconciler(s.Vacancies, s.Applications, s.Conversations, s.Knowledge)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
