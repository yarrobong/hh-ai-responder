...
func TestDashboardSendDryRunReachesGatewayAndNeverTransport(t *testing.T) {
	...
	if !reflect.DeepEqual(types, []string{"send_ui_clicked", "send_api_received", "gateway_invoked", "capability_blocked", "send_failed"}) {
...
func TestDashboardDraftCallsOrchestratorWithoutHHSending(t *testing.T) {
	...
	*s.Knowledge = *kb
	...
	s.Orchestrator = NewAIReplyOrchestrator(ai, s.Conversations, s.Resolver, s.Drafts, s.Clarifications, s.ProposalUpdater)
