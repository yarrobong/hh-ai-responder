...
func runHHCommand(args []string, cfg Config, stdout, stderr io.Writer) error {
	...
	conversations, applications, vacancies, clarifications, drafts, err := loadHHLocalStores(wd, profilePath)
	if err != nil {
		return err
	}
	...
	case "sync":
		if len(args) == 3 && args[1] == "conversation" && strings.TrimSpace(args[2]) != "" {
			...
			if syncErr == nil && len(result.Errors) == 0 {
				...
				if changed, reconcileErr := clarifications.ReconcileCandidateClarifications(conversations, resolver); reconcileErr != nil {
	...
	case "inbox":
		service := NewHHReadSyncService(nil, conversations, clarifications, drafts)
	...
	case "draft":
		...
		orchestrator := NewAIReplyOrchestrator(ai, conversations, resolver, drafts, clarifications, semanticRetriever)
...
func runHHActionPreflight(cfg Config, wd, actionID string, out io.Writer) error {
	...
	conversations, applications, _, clarifications, drafts, err := loadHHLocalStores(wd, profilePath)
	if err != nil {
		return err
	}
	var resolver *CandidateContextResolver
...
func runReconcileCommand(cfg Config, dryRun bool, out io.Writer) error {
	...
	applications := NewApplicationStore(filepath.Join(wd, JobApplicationsFilename), conversations)
...
func runMonitorCommand(cfg Config, out io.Writer) error {
	...
	lock, err := acquireLocalProcessLock(filepath.Join(wd, ".career-monitor"), 2*time.Hour)
	if err != nil {
		return err
	}
	defer lock.Release()
	...
	conversations, applications, vacancies, clarifications, drafts, err := loadHHLocalStores(wd, cfg.CandidateProfilePath)
	if err != nil {
		return err
	}
	...
	syncService := NewHHReadSyncService(NewHHAIResponderReadClient(responder), vacancies, applications, conversations, NewVacancyAnalyzer(), kb, clarifications, drafts, cfg.HHSyncStatePath)
	...
	monitor := NewCareerMonitor(syncService, NewCareerDataReconciler(vacancies, applications, conversations, kb), engine, applications, conversations, monitorStatePath(wd), cfg.MonitorInterval)
	monitor.QuietHours = cfg.MonitorQuietHours
	if err := monitor.LoadState(); err != nil {
		return err
	}
	if cfg.RunOnce {
		if err := monitor.RunOnce(ctx, time.Now().UTC()); err != nil {
			return err
		}
		return writeJSON(out, map[string]any{"state": monitor.State, "notifications": notifications.List()})
	}
...
func buildHHReadSyncService(ctx context.Context, cfg Config, client HHReadClient, vacancies *VacancyStore, applications *ApplicationStore, conversations *ConversationStore, dependencies ...any) (*HHReadSyncService, func(), error) {
	backend, err := normalizeStorageBackend(cfg.StorageBackend)
	if err != nil {
		return nil, func() {}, err
	}
	...
	if backend == storageBackendPostgres {
		return NewHHReadSyncServiceWithRepositories(client, career, dependencies...), closeCareer, nil
	}
	allDependencies := append([]any{vacancies, applications, conversations}, dependencies...)
	return NewHHReadSyncService(client, allDependencies...), closeCareer, nil
}
