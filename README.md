...
Оценка обязательных требований использует три состояния: явное противоречие попадает в `hard_requirements_missing`, недостаток данных — в `hard_requirements_unknown`, а подтверждённое соответствие не попадает ни в один из этих массивов. Отсутствие навыка, образования или локации в переданном контексте не доказывает их отсутствия. Поэтому неизвестная локация, образование или длительность опыта требуют ручной проверки; длительность опыта не вычисляется и не округляется моделью. Фразы вакансии `Без опыта` и `Опыт не требуется` не являются максимальным ограничением опыта.
