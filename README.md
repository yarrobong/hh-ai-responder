...
Перед письмом приложение загружает описание вакансии, применяет deterministic-фильтры и просит AI оценить соответствие кандидату. Отклик продолжается только при `apply=true` и score не ниже `HH_MIN_MATCH_SCORE`. `HH_EXCLUDE_KEYWORDS` и недостаточная известная зарплата отбрасывают вакансию до AI; `HH_INCLUDE_KEYWORDS` является только дополнительным позитивным сигналом. Ключевые слова разделяются запятыми, сравнение нечувствительно к регистру, пустые элементы игнорируются.
