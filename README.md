...
`HH_RUN_ONCE=true` выполняет один проход каждой разрешённой задачи (поднятие резюме, обновление статуса, вакансии и чаты) и завершает программу. В сочетании с `HH_DRY_RUN=true` все HH write-запросы блокируются. За один проход по умолчанию обрабатываются не более 20 eligible-вакансий и не более 10 откликов; в dry-run лимит откликов считает события `application_preview` как `WOULD APPLY`. После прохода вакансий записывается событие `run_summary` с `search_profiles` и полями `vacancies_fetched_raw`, `vacancies_after_dedup`, `duplicates_skipped`, а также совместимыми `vacancies_fetched`, `vacancies_processed` и `vacancies_seen`. Дедупликация по vacancy ID выполняется до AI evaluation и общего лимита `HH_MAX_VACANCIES_PER_RUN`; already-responded state применяется к объединённому списку. Вакансии с непроверяемыми обязательными требованиями получают событие `vacancy_review_required`, не считаются matched и не отправляются автоматически.
